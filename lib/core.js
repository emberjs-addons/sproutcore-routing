...
/**
  ...
  Route is a class used internally by Em.routes. The routes defined by your
  application are stored in a tree structure, and this is the class for the
  nodes.
*/
var Route = Em.Object.extend(
...
*/
var routes = Em.routes = Em.Object.create(
  /** @scope Em.routes.prototype */{
  ...
  */
  ...
  _extractLocation: function(key, value) {
    ...
    if (value !== undefined) {
      ...
      if (!this._skipPush && (!Em.empty(value) || (this._location && this._location !== value))) {
    ...
    if (!this._didSetup) {
      ...
      if (get(this, 'wantsHistory') && supportsHistory) {
        ...
        if (supportsHashChange) {
          ...
          // we don't use a Em.Timer because we don't want
  ...
  */
  add: function(route, target, method) {
    if (!this._didSetup) {
      Em.run.once(this, 'ping');
    }

    if (method === undefined && Em.typeOf(target) === 'function') {
      method = target;
      target = null;
    } else if (Em.typeOf(method) === 'string') {
